<template>
  <footer class="w-full h-40 px-5 mt-10 bg-purple-900/80">
    <div class="flex items-center justify-between max-w-5xl mx-auto">
      <div class="flex flex-col my-4 md:flex-row font-exo text-white/50">
        <div>
          <h5 class="font-bold text-white/75">Contact Us</h5>
          <p class="text-sm"><a href="https://wa.me/918078741585">+91 80787 41585</a></p>
          <p class="text-sm"><a href="https://wa.me/918113946710">+91 81139 46710</a></p>
        </div>
        <div class="mt-3 md:mt-0 md:ml-10">
          <h5 class="font-bold text-white/75">Follow Us</h5>
          <div class="flex mt-2">
            <a href="https://instagram.com/element_mcet">
              <img
                src="../assets/instagram-logo-white.png"
                alt="instagram link to element association"
                class="h-5 opacity-90"
              />
            </a>
            <a href="https://malabarcet.ac.in">
              <img
                src="../assets/globe-logo-white.png"
                alt="instagram link to element association"
                class="h-5 ml-2 opacity-90"
              />
            </a>
          </div>
        </div>
      </div>
      <img
        src="../assets/malabar-logo.png"
        alt="malabar college logo"
        class="w-[12rem] opacity-80"
      />
    </div>
  </footer>
</template>
